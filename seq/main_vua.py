...
pos_set = set()
raw_train_vua = []
with open('../datasets/VUAsequence/VUA_train.csv', encoding='latin-1') as f:
    lines = csv.reader(f)
    next(lines)
    for line in lines:
...
raw_val_vua = []
with open('../datasets/VUAsequence/VUA_val.csv', encoding='latin-1') as f:
    lines = csv.reader(f)
    next(lines)
    for line in lines:
        pos_seq = ast.literal_eval(line[4])
...
#RNNseq_model = RNNseq_model.cuda(device)
if using_GPU:
...
for epoch in range(num_epochs):
    ...
    if performance_matrix['VERB'] > f1_best:
        f1_best = performance_matrix['VERB']
        save_model(RNNseq_model, epoch)
        print('verb f1',performance_matrix['VERB'])
...
raw_test_vua = []
with open('../datasets/VUAsequence/VUA_test.csv', encoding='latin-1') as f:
    lines = csv.reader(f)
    next(lines)
    for line in lines:
...
test_rnn = RNNSequenceModel(num_classes=2, embedding_dim=300 + 64, hidden_size=300, num_layers=1,
                                    bidir=True,
                                    dropout1=0.3, dropout2=0.2, dropout3=0.2)

# model = model.cuda(device=device_ids[0])
if using_GPU:
    test_rnn = test_rnn.cuda()
test_rnn = load_model(test_rnn)
...
test_dataloader_vua = DataLoader(dataset=test_dataset_vua, batch_size=batch_size,
                                     collate_fn=TextDataset.collate_fn)
avg_eval_loss, performance_matrix = test(idx2pos, test_dataloader_vua, test_rnn,
                                                         loss_criterion, using_GPU)
